"""Write all 4 regenerated testbench files to tb directory.
Fixes:
- Uses STD.ENV.ALL + STOP for clean simulation termination
- Proper self-checking with IF/ELSE (no misleading ASSERT + REPORT combos)
- Fixed timing generator TB timing (check during reset, correct edge counting)
"""
import os
...
BEGIN
  ...
  BEGIN
    ...
    -- TIM-T-001: Reset clears all counters
    s_rst <= '1';
    WAIT FOR 5 * c_clk_period;
    -- Check WHILE reset is active (counters should be cleared)
    WAIT FOR 1 ns;
    s_test_count <= s_test_count + 1;
    v_pass := (s_bit_count = "00000") AND
              (s_word_type = '0') AND
              (unsigned(s_op_count) = 0);
    IF v_pass THEN
      REPORT "TIM-T-001: Reset clears counters PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-001 FAILED: counters not 0 during reset, bit_count=" &
        INTEGER'image(to_integer(unsigned(s_bit_count))) SEVERITY ERROR;
    ...
    -- After reset deassert, first rising edge increments bit_cnt to 1
    -- After 19 edges total, bit_cnt = 19
    s_test_count <= s_test_count + 1;
    FOR i IN 0 TO 18 LOOP
      WAIT UNTIL rising_edge(s_clk_master);
    END LOOP;
    WAIT FOR 1 ns;
    IF unsigned(s_bit_count) = 19 THEN
      REPORT "TIM-T-002: Bit counting (T0-T19) PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-002 FAILED: bit_count=" &
        INTEGER'image(to_integer(unsigned(s_bit_count))) &
        " expected 19" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;

    -- TIM-T-003: WA/WO alternation (word boundary -> word_type toggles)
    -- One more edge: bit_cnt wraps to 0, word_type goes to '1' (WO)
    WAIT UNTIL rising_edge(s_clk_master);
    WAIT FOR 1 ns;
    s_test_count <= s_test_count + 1;
    v_pass := (s_bit_count = "00000") AND (s_word_type = '1');
    IF v_pass THEN
      REPORT "TIM-T-003: WA/WO alternation PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-003 FAILED: bit=" &
        INTEGER'image(to_integer(unsigned(s_bit_count))) &
        " word_type=" & std_logic'image(s_word_type) SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    s_rst <= '0';
    s_test_count <= s_test_count + 1;
    -- Wait 18 edges to reach bit_cnt=18
    FOR i IN 0 TO 17 LOOP
      WAIT UNTIL rising_edge(s_clk_master);
    END LOOP;
    WAIT FOR 1 ns;
    IF s_word_mark = '1' THEN
      REPORT "TIM-T-004: Word_mark at T18 PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-004 FAILED: word_mark not asserted at T18" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    WAIT UNTIL rising_edge(s_clk_master);
    WAIT FOR 1 ns;
    IF s_word_mark = '0' THEN
      REPORT "TIM-T-004b: Word_mark deasserted at T19 PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-004b FAILED: word_mark still asserted at T19" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    s_rst <= '0';
    s_test_count <= s_test_count + 1;
    -- One OP = 40 edges (20 WA + 20 WO)
    FOR i IN 0 TO 39 LOOP
      WAIT UNTIL rising_edge(s_clk_master);
    END LOOP;
    WAIT FOR 1 ns;
    IF unsigned(s_op_count) = 1 THEN
      REPORT "TIM-T-005: Operation counting PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-005 FAILED: op_count=" &
    ...
    END LOOP;
    WAIT FOR 1 ns;
    IF s_frame_mark = '1' THEN
      REPORT "TIM-T-006: Frame mark PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-006 FAILED: frame_mark not asserted" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    WAIT FOR 1 ns;
    s_test_count <= s_test_count + 1;
    v_pass := (unsigned(s_op_count) = 0) AND (s_frame_mark = '0');
    IF v_pass THEN
      REPORT "TIM-T-007: Frame wrap PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-007 FAILED: op_count=" &
        INTEGER'image(to_integer(unsigned(s_op_count))) SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;

    -- TIM-T-008: Phase clocks are valid logic levels
    s_rst <= '1';
    WAIT UNTIL rising_edge(s_clk_master);
    s_rst <= '0';
    WAIT UNTIL rising_edge(s_clk_master);
    WAIT FOR 1 ns;
    s_test_count <= s_test_count + 1;
    IF (s_phi1 = '1' OR s_phi1 = '0') AND (s_phi2 = '1' OR s_phi2 = '0') THEN
      REPORT "TIM-T-008: Phase clock generation PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-008 FAILED: phi1/phi2 undefined" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    WAIT FOR 1 ns;
    s_test_count <= s_test_count + 1;
    v_pass := (s_bit_count = "00000") AND (unsigned(s_op_count) = 0);
    IF v_pass THEN
      REPORT "TIM-T-009: Reset during operation PASSED" SEVERITY NOTE;
    ELSE
      REPORT "TIM-T-009 FAILED: counters not 0 on mid-op reset" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    s_rst <= '0';
...
BEGIN
  ...
  stim_proc : PROCESS
    ...
    PROCEDURE do_multiply(
      ...
      CONSTANT b         : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT exp_r     : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT name      : IN STRING;
      CONSTANT tolerance : IN INTEGER := 1
    ) IS
    ...
    BEGIN
      ...
      IF v_timeout >= 50 THEN
        REPORT name & " FAILED: done never asserted (timeout)" SEVERITY ERROR;
        s_fail_count <= s_fail_count + 1;
      ELSE
  ...
  BEGIN
    ...
    s_test_count <= s_test_count + 1;
    IF s_busy = '1' THEN
      REPORT "PMU-T-020: Busy asserted PASSED" SEVERITY NOTE;
    ELSE
    ...
    WAIT FOR 1 ns;
    IF s_busy = '0' THEN
      REPORT "PMU-T-020b: Busy deasserted after done PASSED" SEVERITY NOTE;
    ELSE
      REPORT "PMU-T-020b FAILED: busy not deasserted" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    s_test_count <= s_test_count + 1;
    IF s_done = '1' THEN
      REPORT "PMU-T-021: Done asserted PASSED" SEVERITY NOTE;
    ELSE
    ...
    WAIT FOR 1 ns;
    IF s_done = '0' THEN
      REPORT "PMU-T-021b: Done deasserted after 1 cycle PASSED" SEVERITY NOTE;
    ELSE
      REPORT "PMU-T-021b FAILED: done not deasserted" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;

    -- PMU-T-023: Back-to-back operations
    do_multiply(x"40000", x"40000", x"20000", "PMU-T-023a: back-to-back 1");
    do_multiply(x"20000", x"40000", x"10000", "PMU-T-023b: back-to-back 2");
...
BEGIN
  ...
  stim_proc : PROCESS
    ...
    PROCEDURE load_acc(
      CONSTANT v_val : IN std_logic_vector(19 DOWNTO 0)
    ) IS
    BEGIN
      s_alu_op         <= c_op_load;
      s_acc_in         <= v_val;
    ...
    PROCEDURE check_alu(
      ...
      CONSTANT operand  : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT expected : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT name     : IN STRING
    ) IS
    BEGIN
      s_alu_op         <= op;
  ...
  BEGIN
    ...
    s_test_count <= s_test_count + 1;
    IF s_flag_c = '1' THEN
      REPORT "SLF-T-003b: carry flag PASSED" SEVERITY NOTE;
    ELSE
      REPORT "SLF-T-003b: carry flag FAILED" SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    load_acc(x"00000");
    check_alu(c_op_gray2bin, x"00000", x"00000", "SLF-T-040: Gray->Bin 0");

    -- SLF-T-041: Gray->Bin: 0x00001
    load_acc(x"00001");
    check_alu(c_op_gray2bin, x"00000", x"00001", "SLF-T-041: Gray->Bin 1");

    -- SLF-T-042: Gray->Bin: 0x00003 -> 0x00002
    load_acc(x"00003");
    check_alu(c_op_gray2bin, x"00000", x"00002", "SLF-T-042: Gray->Bin 3->2");
    ...
    ELSE
      REPORT "SLF-T-044 FAILED: got 0x" & to_hstring(unsigned(s_acc_out))
        SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
    ...
    ELSE
      REPORT "SLF-T-056 FAILED: tmp=0x" & to_hstring(unsigned(s_tmp_out))
        SEVERITY ERROR;
      s_fail_count <= s_fail_count + 1;
    END IF;
...
BEGIN
  ...
  stim_proc : PROCESS
    ...
    PROCEDURE check_mux(
      CONSTANT actual   : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT expected : IN std_logic_vector(19 DOWNTO 0);
      CONSTANT name     : IN STRING
    ) IS
    BEGIN
...
print("\nAll 4 testbenches written successfully.")
