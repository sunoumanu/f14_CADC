#-------------------------------------------------------------------------------
# F-14A CADC Block Design Creation Script — Arty S7-50
...
#
# Identical topology to the CmodA7 design; only clock input frequency differs.
#-------------------------------------------------------------------------------

# Create block design
create_bd_design "cadc_system"

# Create clock port (12 MHz from Arty S7-50)
create_bd_port -dir I -type clk -freq_hz 12000000 sys_clk
set_property CONFIG.FREQ_HZ 12000000 [get_bd_ports sys_clk]

# Create reset port (btn[0], active-low press)
